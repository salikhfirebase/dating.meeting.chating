...
import dating.meeting.chating.PersonalRazvaliliActivity
 ...
 *
 */
class SingInSsdfFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        val view = inflater.inflate(R.layout.fragment_sign_in_ssdf, container, false)

        db = AppDatabase.getInstance(this.requireContext()) as AppDatabase
        the_emailEditText = view.findViewById(R.id.email_sing_into_caps_por_edit_text)
        the_passEditText = view.findViewById(R.id.password_sign_dfgd_edit_text_user)
        the_logInButton = view.findViewById(R.id.sign_in_button_df_wer_sign_in_screen)
    ...
    fun isUserInDb(email: String, password: String) {
        ...
        Observable.fromCallable{ db.userDao().verifaicate(email,password) }
            ...
            .subscribe {
                if (it>0) {
                    the_intent1 = Intent(this.requireContext(), PersonalRazvaliliActivity::class.java)
